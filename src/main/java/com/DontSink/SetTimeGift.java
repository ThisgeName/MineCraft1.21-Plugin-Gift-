...
public class SetTimeGift implements CommandExecutor {
    @Override
    ...
    public void setTimeGift(Player p, String a1, String a2) {
        try {
            ...
            String newEntry = a1 + ".time_" + a2;
            boolean replaced = false;

            // 检查是否存在相同的条目并替换
            for (int i = 0; i < lines.size(); i++) {
                if (lines.get(i).startsWith(a1 + ".time_")) {
            ...
            // 向玩家发送消息，通知内容已保存
            p.sendMessage("已设定" + a1 + "礼包的冷却时间为"+a2+"分钟");
        } catch (IOException e) {
            // 如果发生异常，向玩家发送错误消息，并打印堆栈跟踪
            p.sendMessage("设定冷却时间出错: " + e.getMessage());
            e.printStackTrace();
        }
